...
const Referral: React.FC = () => {
    ...
    return (
        <div className="min-h-screen pt-32 pb-20 px-6 max-w-3xl mx-auto">
            <div className="bg-black border border-white/10 p-12 text-center relative overflow-hidden mb-12">
                ...
                <div className="relative z-10">
                    ...
                    <h2 className="text-4xl font-bold text-white mb-4 uppercase tracking-tighter">Invite & Earn</h2>
                    ...
                    <div className="bg-white/5 border border-white/20 p-2 flex items-center justify-between max-w-md mx-auto mb-8">
                        <code className="text-xl font-mono text-cyan-400 font-bold px-4">VOYAGE-X92</code>
                        <button className="px-6 py-3 bg-white text-black font-bold uppercase text-xs tracking-wider hover:bg-cyan-400 transition-colors flex items-center gap-2">
                            <Copy className="w-4 h-4" /> Copy
                        </button>
                    </div>

                    <div className="flex justify-center gap-4">
                        <button className="p-4 border border-white/10 hover:bg-white hover:text-black transition-colors">
            ...
            <div className="border-t border-white/10 pt-12">
                ...
                <div className="bg-black border border-white/10">
                    ...
                    {history.map((item, idx) => (
                        <div key={idx} className="grid grid-cols-4 p-4 border-b border-white/5 text-sm font-mono text-zinc-300 last:border-0 hover:bg-white/5 transition-colors">
                            <span>{item.email}</span>
                            <span>{item.date}</span>
                            <span className={item.status === 'Joined' ? 'text-emerald-400' : 'text-orange-400'}>{item.status}</span>
                            <span className="text-right font-bold text-white">{item.reward}</span>
                        </div>
                    ))}
                </div>
            </div>
