    ...
    return (
        <div className="min-h-screen pt-32 pb-20 px-6 max-w-7xl mx-auto">
            <div className="text-center mb-16">
                <span className="text-cyan-400 font-mono text-sm uppercase tracking-wider mb-4 block">Global Network</span>
                <h2 className="text-4xl md:text-6xl font-bold text-white mb-6 uppercase tracking-tight">Voyageur Community</h2>
                <p className="text-slate-400 max-w-2xl mx-auto">Explore curated itineraries from our top explorers.</p>
            </div>
            ...
            <div className="mb-20">
                ...
                <div className="grid md:grid-cols-3 gap-6">
                    {creators.map((c, i) => (
                        <div key={i} className="bg-black border border-white/10 p-6 flex items-center gap-4 hover:border-cyan-400/50 transition-colors group cursor-pointer">
                            <img src={c.img} alt={c.name} className="w-16 h-16 rounded-full border-2 border-black grayscale group-hover:grayscale-0 transition-all" />
                            <div>
                                <h4 className="font-bold text-white text-lg uppercase">{c.name}</h4>
                                <div className="text-xs text-cyan-400 font-mono mb-1">{c.role}</div>
                                <div className="text-xs text-zinc-500 font-bold uppercase tracking-wider">{c.followers} Followers</div>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div className="grid md:grid-cols-3 gap-8 mb-20">
                {posts.map((post, idx) => (
                    <div key={idx} className="bg-black border border-white/10 group hover:border-cyan-400/50 transition-all cursor-pointer">
                        <div className="h-64 overflow-hidden relative">
                            <img src={post.image} alt={post.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                            <div className="absolute top-4 left-4 flex gap-2">
                                {post.tags.map(tag => (
                                    <span key={tag} className="bg-black/80 backdrop-blur border border-white/20 text-white text-[10px] font-bold px-2 py-1 uppercase tracking-wide">{tag}</span>
                                ))}
                            </div>
                        </div>
                        <div className="p-6">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    <div className="text-xs font-bold text-white uppercase">{post.user}</div>
                                    <div className="text-[10px] text-zinc-500 font-mono flex items-center gap-1">
                                        <MapPin className="w-3 h-3" /> {post.location}
                                    </div>
                                </div>
                            </div>
                            <h3 className="text-xl font-bold text-white mb-4 uppercase leading-tight group-hover:text-cyan-400 transition-colors">{post.title}</h3>
